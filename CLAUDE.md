...
**Historical snapshots (`member_analysis_history` table), added 2026-07-24 — migration run:**
```sql
...
Goal columns — **migration run** (confirmed live in production, e.g. Happy Valley has real `goal_count_annual`/`goal_premium_annual` values):
```sql
...
**Combined product goals (added 2026-09-04, migration NOT yet run as of this writing — `Old SQLs/location-combined-product-goals-migration.sql`)**:
```sql
...
```
Same `{id, label, products, target}` shape as an agent goal's `combined_groups` entries (e.g. "Auto+Fire: 15") — two independent arrays since monthly/annual product goals are already two independent maps on this same row. Requested so a location's Office Goals can group products the same way individual/team/agency goals already could. UI: `_buildLocCgRow`/`_addLocCgRow`/`_removeLocCgRow` (`js/sales.js`), rendered under both the Monthly and Annual Product Goals sections in the location edit form; server-side `sanitizeCombinedProductGoals()` (`api/checklist-config.js`, mirrors `sanitizeThresholdTiers`'s never-trust-the-client-array posture). Display: static, in `_renderAgencyGoalsSection()`'s existing per-product row style (`js/goals.js`) — no actual-vs-target computation, matching the *existing* flat `product_goals_monthly`/`product_goals_annual` display, which was likewise always static (confirmed by reading `_renderSlScorecard` too — even the pre-existing flat per-product location goals have never had progress-pill computation anywhere in the app, so this stays consistent rather than introducing an inconsistent partial feature).

**Graceful degradation until the migration runs**: both the `GET` location SELECT and the `update_details` PATCH retry once without the two new columns on error, so every *other* location field (address, the pre-existing flat goals, activity goals) keeps saving/loading normally in the meantime — an unknown-jsonb-column error on a single combined UPDATE/SELECT would otherwise have broken the *entire* location record, not just the new fields. Verified live pre-migration: GET correctly falls back to `combined_product_goals_monthly/annual: []`; PATCH with a combined-goal row included still saves `goal_count` etc. correctly while silently dropping just the new fields.

**Where "Blended" raise-goal comparisons actually pull from — a common point of confusion, since there are two separate "agency goal" concepts in this app.** A raise-eligible goal's Blended/Separate `agency_location_id` (see "Annual Raise-Eligibility Tracker" above) reads targets from **this table** — `sales_locations.goal_count`/`goal_premium`/`goal_count_annual`/`goal_premium_annual`, i.e. the per-location Office Goals set in Account → Sales → Locations → Edit — when a specific location or "All Locations" is selected. It does **not** read from the separate "Whole Agency" `agent_goals` sentinel row (Account → Sales → Team → Team & Agency Goals) that way. The "Agency Location" and "Combination Mode" tooltips in the raise-goal authoring form (`js/goals.js`, `buildRaiseGoalHtml`) state this explicitly, added after a report asking "confirm where the blended goal is grabbing from" surfaced that this wasn't obvious from the UI alone.

**"Whole Agency Goal" source (added 2026-09-04)** — a third option, `WHOLE_AGENCY_GOAL_SENTINEL = '__whole_agency_goal__'` (`api/_lib/raise-calc.js`), for exactly the case the tooltip above couldn't satisfy: aligning an individual's raise with the agency's own **raise-eligible** goal, not a plain `sales_locations` sales target. Explicit product intent, stated directly: *"if individual hits goal, they're eligible for raise, but won't be as large if agency misses set annual goals; if agency goals are met but individual misses goals then not eligible."* This is expressed with existing mechanics, not new reward math — set **Individual Gate = 100%** (individual must fully clear their own goal to be eligible at all — satisfies both "not eligible if individual misses" and, implicitly, "eligible if individual hits") and **Combination Mode = Blended** sourced from Whole Agency Goal (once past the gate, the blended score — and therefore the reward size — scales down when the agency's own goal is behind, satisfies "won't be as large if agency misses").

Implementation (`attachRaiseStatus()`, `api/agent-goals.js`): the Whole Agency's own `agent_goals` row (`agent_id: '__agency__'`) matching the individual goal's `period_type` and flagged `is_raise_goal: true` is looked up from the SAME already-`computeActuals()`'d batch (`wholeAgencyGoalByPeriodType`, built once) — its progress is computed by directly reusing `computeIndividualProgressPct()` on it, the exact generic "mean of actual/target ratios" function every goal already gets, not a new formula. No new DB queries beyond what the batch already fetches. `computeAgencyProgressPct()`/`sales_locations` queries are untouched and only run for the other two sources. Server-side location-existence validation (POST/PATCH) skips both `ALL_LOCATIONS_SENTINEL` and `WHOLE_AGENCY_GOAL_SENTINEL` the same way.

**Agency Metric (count-vs-premium either/or) removed — blended instead (fixed 2026-09-04).** Reported as "agency and agents need to have premium goals not just count goals" — the location/All-Locations comparison forced a single "Agency Metric" choice (Policy Count OR Premium), unlike an individual's own goal, a team goal, or the Whole Agency Goal source above, all of which already blend *every* metric they have set (mean of ratios). `computeAgencyProgressPct()` (`api/_lib/raise-calc.js`) now does the same: averages whichever of `goal_count(_annual)`/`goal_premium(_annual)` the location actually has set, instead of requiring one or the other. The now-redundant "Agency Metric" dropdown and `toggleRaiseLocFields()` were removed from the form; `raise_config.agency_metric` is kept as a harmless vestigial field so an already-saved config isn't stripped, but nothing reads it anymore. Verified live: a location with a 40-policy / $20,000-premium annual goal against real actual sales of 2 policies / $470 premium computed `agency_pct: 3.68` — matching an independent hand-calc of `(2/40 + 470/20000) / 2 × 100` exactly.

**Combined product goals gained an independent premium target (fixed 2026-09-04).** Follow-up clarification from the same report, with a concrete example: *"combined goal for one agent may be 40 life policies and 100k in premium while another may be 65 life policies and 25k in premium"* — confirmed this was never in doubt for the *plain* (non-combined) Total Policies / Premium ($) fields (already fully independent per agent, verified live by saving exactly those two example goals for two different agents with zero cross-influence), but the *Combined Goals* feature (grouping products into one target, e.g. "Auto+Fire: 15") only ever had a single numeric `target` — no way to also set a premium target on the same group. `combined_groups` entries gained an optional `target_premium` field, independent of `target` exactly like the plain fields (either or both may be set) — same extension applied to both `agent_goals.goals.combined_groups` (individual/team/agency) and `sales_locations.combined_product_goals_monthly/annual` (locations), since both share the identical `{id, label, products, target}` shape. `computeActuals()` now also sums `actuals['combined_'+id+'_premium']` when `target_premium` is set; `computeIndividualProgressPct()` includes it as one more independent ratio in the mean. Every display site that reads a combined group (`renderGoalCards`, the Race-tab quick-view, the roster-card pill summary, `_renderAgencyGoalsSection`'s static location display) was updated to show both — the previous `if (!grp.target) continue` guards would have silently hidden a premium-only combined group entirely, now fixed to check either field. Verified live: a combined "Auto+Fire" group with `target:15, target_premium:50000` correctly persisted and computed both `actuals.combined_cg0` and `actuals.combined_cg0_premium`, for both an individual agent goal and a location's combined product goals.

Verified live end-to-end (`vercel dev` + Chrome, real magic-link session — the dev server had died again mid-session and needed restarting): created a real Whole Agency annual raise goal (policies target 100, actual 2 → 2%) and a real individual annual raise goal (Blended, Whole Agency Goal source, 70/30 weight, Gate @ 100%) — confirmed `raise_status.agency_pct: 2` exactly matches the whole-agency goal's own 2/100, proving the source is genuinely the raise-eligible agent_goals row and not `sales_locations`; confirmed `gate_passed: false` / `earned_pct: 0` with the individual at 0% despite a nonzero blended score, matching the "not eligible if individual misses" rule exactly. Confirmed the authoring form re-populates the saved selection correctly and hides Agency Metric for this source. Test goals deleted after verification.

### Condensed Whole Agency progress bar, pinned to the app header (added 2026-09-04)

A Whole Agency goal (any period type, doesn't need to be raise-eligible) can be pinned as a slim, color-coded progress bar in the app header — positioned between the title block and the LIVE/email/Sign Out column (`index.html`, `#header-agency-progress`, third child of the flex `.header` so `justify-content:space-between` naturally sits it in between). Account-wide setting, not a per-viewer preference — one owner-set pin shows for everyone who loads the app, same visibility model as any other goal.

**Data model**: `agent_goals.show_in_header boolean` (`Old SQLs/goal-show-in-header-migration.sql`, confirmed run 2026-09-04). Enforced in application code, not a DB constraint: only settable when `agent_id === '__agency__'` (POST/PATCH both reject otherwise), and only one goal account-wide may have it `true` at a time — `unpinOtherHeaderGoals()` unsets it on every other `__agency__` goal whenever a save sets it to `true`. Graceful degradation on both POST and PATCH (retry without the field on an unknown-column error) matches this session's established pattern for new jsonb/boolean columns rolled out mid-session.

**Backend computation** — `attachHeaderProgress()` (`api/agent-goals.js`, called unconditionally in the `GET ?withActuals=1` handler, right after `attachRaiseStatus`): attaches a `header_progress: {progress_pct, progress_color, projected_pct, projected_color}` object to whichever `__agency__` goal(s) have `show_in_header: true`. Deliberately **not** gated on `is_raise_goal` or `isRaiseEligiblePeriod` — this needed to work for any Whole Agency goal regardless of period type or raise status, unlike `attachRaiseStatus`. Reuses the exact same pure math as the raise-eligibility feature (`computeIndividualProgressPct`/`computeMonthsElapsed`/`annualizedPct`/`colorForYtd`/`colorForAnnualized`) rather than inventing a parallel formula — "progress toward goal" and "pace-projected progress" mean the same thing they already do on an individual's own raise card.

**Frontend**: the "📌 Show in header" checkbox only renders in the goal-authoring form (`buildGoalMetricsHtml`'s sibling row in `showGoalForm`, `js/goals.js`) when `agentId === '__agency__'`. `renderHeaderAgencyProgress()` derives the pinned goal directly from `_agentGoals` — already fetched for every logged-in user at login by the pre-existing `loadAgentGoals()` (no new endpoint or separate request needed) — and respects the same Public/Private visibility a non-writer already gets on the Goals tab (`canPrivate` check; a private pinned goal only shows for writers). The **OT checkbox** in the header bar itself (`#hap-ot`, client-side only, not persisted — same ephemeral-toggle pattern as an individual's own "Show annualized projection" checkbox) switches the displayed number and color between `progress_pct`/`progress_color` (raw cumulative, default) and `projected_pct`/`projected_color` (pace-projected to 100% of the goal's period, labeled "OT" for On Track). Saving a Whole Agency goal always triggers a full `loadAgentGoals()` refetch rather than a local cache patch, since `header_progress` is only ever computed server-side.

Verified live end-to-end: pinned a real annual Whole Agency goal (policies target 100, actual 2) — confirmed `header_progress: {progress_pct: 2, progress_color: "red", projected_pct: 2.96, projected_color: "red"}`, the header DOM correctly showed a 2%-wide red bar labeled "Whole Agency · 2026", and toggling OT switched it to "3% projected" at 2.96% width — both exactly matching the server response. Pinned a second (quarterly) Whole Agency goal and confirmed the first was automatically unpinned (exactly one `show_in_header: true` row afterward). Confirmed the privacy gate: simulating a bosun session (with `_isAdmin` also correctly overridden, not just `_isMember`/`_memberRole` — an early test attempt missed this and produced a false pass) correctly hid a private pinned goal's header bar, while the owner/admin still saw it. Also found and cleaned up an unrelated stray `__agency__` test goal left over from earlier in the same session (a real gap in that turn's own cleanup, not a bug in this feature). All test goals deleted after verification.
...
`saveLocationDetails()` reads all goal fields and sends `goal_count_annual`, `goal_premium_annual`, `goals_visibility`, `product_goals_monthly`, `product_goals_annual` to `PATCH /api/checklist-config` action `update_details`. Requires the 5 `sales_locations` goal columns (see "sales_locations columns" above — migration confirmed run).
...
**Hit again 2026-09-03**: the Team/Agency Goals authoring panel (`_renderScopeGoalsPanel()`, see "Call-Metric Goals" below) was originally wired into `renderAgentRoster()` in `js/sales.js` — the dead one — so it never appeared under Account → Sales → Team despite testing the rest of that feature live in Chrome the same day (the Chrome pass happened to land on a state where the panel wasn't checked directly). Reported as "I don't see the agency goals that was just built." Fixed by rendering `_renderScopeGoalsPanel()` inside `AgentRosterIsland()` itself (`js/roster-island.js`), via the same `dangerouslySetInnerHTML` pattern as the per-agent cards, prepended before the roster map. **This warning bears repeating because it was already documented here once and still got missed** — before wiring anything new into "the Agent Roster," grep `js/roster-island.js` first to confirm where it actually needs to go.
...
Migration confirmed run/live as of 2026-09-02. Every read/write path still degrades gracefully if it somehow hasn't run on a given account (e.g. a future new environment mid-bootstrap): the GET handler retries the payments query without `installments` on error (this table is shared across every account on the platform — one missing migration must not break commissions for everyone), the full-record upsert retries without the `installments` field on the same error, and the client falls back to synthesizing a single installment from the legacy fields when `installments` is empty. Only `addInstallment` itself hard-requires the column (there's no meaningful degraded behavior for "add a second payment" without anywhere to put it) — kept intentionally, not because the column is currently missing.
...
**Threshold bonuses (`threshold_tiers jsonb`, default `'[]'`)** — migration run:
```sql
...
**Two new `HELP_GUIDES` entries** (`js/help.js`): `raise-eligibility-member` (`visibility: ['all']`) and `raise-eligibility-owner` (`visibility: ['owner']`). Originally text-only steps; enriched 2026-09-04 with illustrated worked examples (cumulative month-strip, combination-mode dual-bars, proportional-reward formula, threshold-tier ladder, individual-gate danger callout, annualize-projection formula) matching the visual teaching style of the original `raise-goal-mockup` design-exploration artifact (published earlier in the same design pass, never itself wired into the app) — new `hg-dualbar`/`hg-formula`/`hg-danger`/`hg-months`/`hg-tierladder` CSS classes added to `index.html` alongside the existing `hg-` illustration set for this.

**Verification performed**: every worked example above was checked against the live API (`vercel dev`) with seeded `sales_log`/`sales_locations` test data, not just unit-level — including the exact gate example (40% individual / 95% agency → 56.5% blended, gated to $0 because individual is below the 50% floor) and confirming `separate` mode returns no `earned_pct`/`projected_pct` keys at all. Confirmed a pre-existing non-raise goal still returns unchanged (no `raise_status` key, `actuals` intact) — no regression. **Live UI confirmed 2026-09-03** (Chrome, `vercel dev`): the Raise Eligibility card renders and computes correctly in the real Goals tab (`200% · 4.00% raise earned`, capped/colored correctly). **Still not visually verified**: the individual-goal authoring form's re-population from an existing record, the Help Guide entries' actual rendering, the interactive annualize checkbox's toggle behavior, and a real non-writer member login — these remain correct by code review only.

**Extended: All Locations blending + recurring-monthly eligibility (added 2026-09-04).** Two gaps closed together since both touch the same authoring form and `attachRaiseStatus()` period-scoping code:

- **`ALL_LOCATIONS_SENTINEL = '__all_locations__'`** (`api/_lib/raise-calc.js`, exported) — a new option in Blended/Separate mode's Agency Location dropdown, alongside picking one specific `sales_locations` row. Same non-UUID sentinel-string convention already used for `agent_goals.agent_id` (`__agency__`/`__team_sales__`/`__team_service__`). Aggregates `goal_count(_annual)`/`goal_premium(_annual)` across every `goals_enabled` location as the target, and every location's sales account-wide (unfiltered by `sales_log.location`) as the actual — same "All Locations" semantics `js/sales-log.js`'s Sales Log scorecard already uses. A small ⓘ info icon next to the "Agency Location" label (`js/goals.js`, native `title` tooltip, same pattern already used for the Recurring checkbox) explains what it does. Useful when an agent isn't tied to one office, or the raise should reflect overall agency performance rather than one location's.

- **Raise-eligible goals can now be monthly, if Recurring** (previously annual-only) — a one-off, non-recurring monthly goal has no ongoing pace to track a raise against, so recurrence is required. `isRaiseEligiblePeriod(periodType, isRecurring)` (`api/agent-goals.js`) centralizes the rule, checked in POST, PATCH, and `attachRaiseStatus()` alike (defense in depth — not just trusted from the client). **PATCH auto-clear**: if a save turns off Recurring on an existing monthly raise goal without touching `is_raise_goal` in the same request, the server clears `is_raise_goal`/`raise_config` itself rather than leaving an orphaned invalid state that a fresh save could never have produced — verified live (PATCH `is_recurring:false` alone on an existing monthly raise goal correctly flipped `is_raise_goal` to `false`).

- **Authoring UI** (`js/goals.js`, `buildRaiseGoalHtml`/`updateRaiseSectionVisibility`): the raise section is now shown (not hidden) for both Annual and Monthly period types. For Monthly, the "🎯 Raise-Eligible Goal" checkbox itself is greyed out and `disabled` unless Recurring is checked, with a `title` tooltip explaining why — chosen over hiding the whole section, so the requirement is visible rather than silently vanishing. The Recurring checkbox now has an `onchange` that live-reevaluates this (no need to change Period Type to see the checkbox re-enable).

- **Math generalized, not reimplemented**: `computeMonthsElapsed()` + `annualizedPct()` (`api/_lib/raise-calc.js`) already project-to-100%-of-period regardless of period length — `monthsElapsed = elapsedFraction × 12` and `annualizedPct = driverPct / monthsElapsed × 12` algebraically cancel to `driverPct / elapsedFraction`, so no new math was needed, just permission to reach it for a monthly period. `computeAgencyProgressPct()` gained a `periodType` param so a monthly raise goal blends against a location's MONTHLY `goal_count`/`goal_premium` columns, not its annual ones — comparing a single month's actual sales against a whole year's target would have been silently, severely wrong. Display wording in `_renderRaiseGoalCard()` is now period-aware too: "YTD"/"annualized" only literally means what it says for an annual goal, so a monthly goal shows "MTD"/"projected (this month)" instead.

- **Bonus fix surfaced while generalizing this**: `attachRaiseStatus()` used to read each goal's raw stored `period_start`/`period_end` for both `computeMonthsElapsed` and the agency-location date window — correct only for a NON-recurring goal. A *recurring* goal's effective current period (`_eff_start`/`_eff_end`, already computed by `computeActuals()` via `currentPeriodDates()`) drifts from those raw stored columns after the first cycle: a recurring annual raise goal's stored `period_start` stays pinned to whichever year it was first created, and — far more visibly — a recurring monthly one would have stayed pinned to its first month forever, defeating the entire point of "recurring." Harmless-until-now for the annual case (no raise goal had survived past its first year yet); would have been immediately and obviously broken for the new monthly case if not fixed alongside it. Now uses `periodOf(goal) = { start: g._eff_start || g.period_start, end: g._eff_end || g.period_end }` throughout.

**Verification performed (2026-09-04, live `vercel dev` + Chrome)**: confirmed via the real authoring form that the raise checkbox is disabled/greyed with the correct tooltip for a monthly non-recurring goal, and live-re-enables the instant Recurring is checked (no form reload needed). Saved a real Whole Agency monthly-recurring raise goal in Blended mode with Agency Location = All Locations through the actual UI and confirmed via the API response: `combination_mode: "blended"`, `agency_location_id: "__all_locations__"`, and a fully-populated `raise_status` with correct arithmetic (target = sum of both test locations' monthly `goal_count` = 200; actual = 32 account-wide September policies; `agency_pct: 16` = 32/200 exactly; `annualized_pct: 2288` = `combined_pct(228.8) / months_elapsed(1.2) × 12` exactly). Confirmed server-side rejection for both a non-recurring monthly attempt and a recurring quarterly attempt (still unsupported). Confirmed the PATCH auto-clear case directly. Test goals deleted after verification.

### Call-Metric Goals — handle rate, voicemail count, missed calls (added 2026-09-03)

Extends the goal system above with 3 more trackable metrics — `handle_rate`, `voicemail_count`, `missed_calls` — settable at 3 scope levels: individual agent (existing), a whole team (`sales`/`service`), or the whole agency. Requested specifically so these could feed the raise-eligibility tracker's `is_raise_goal`/`raise_config` just like any other goal metric.

**`handle_rate` didn't exist as a stored metric anywhere before this** — adopted verbatim from the one prior precedent, an unpersisted formula inline in an AI-prompt builder (`api/ai-analysis.js:461-462`): `answered / (answered + voicemail + missed) * 100`, rounded.

**Team/agency scope reuses `agent_goals` via sentinel `agent_id` values** (`__team_sales__`, `__team_service__`, `__agency__`) rather than a new table — a non-agent string in this exact `agent_id`-shaped field is an established pattern already (`'__unlinked_member__'`, used for a different fail-closed purpose in the same file). This gets `is_public`, `is_recurring`, and — most importantly — `is_raise_goal`/`raise_config` for free: a team or agency goal can drive its own raise/bonus determination with zero new `raise-calc.js` code, since that code already operates generically on whatever `goal.actuals`/`goal.goals` it's given.

**"Agency" means the whole account, not a specific `sales_locations` row** — a deliberate deviation from how the existing policy/premium Agency Goal works (which *is* per-location). Forced by a real schema gap: `call_log` has no `location` column, and no agent-to-location link exists anywhere (confirmed both here and in the raise-eligibility research above). Reuses the same whole-account scope `race_wide_missed`/`race_wide_voicemail` already used elsewhere. No cross-scope raise blending was built (e.g. "70% my handle rate + 30% my team's") — team/agency goals independently drive their own raise/bonus via their own sentinel row's own `is_raise_goal` flag; blending against a *different* row would need `raise_config` to reference another `agent_goals` row, real follow-on work, not done here.

**Scope resolver, generalized for every metric, not just the 3 new ones** — `resolveScopeAgentIds()` (`api/agent-goals.js`) turns a goal's `agent_id` into the set of real agent_ids it covers: individual → itself, `__team_*__` → every `agent_roster` agent on that team (falls back to `race_data.team` if the account has no roster — same default-to-`'sales'` assumption used everywhere else), `__agency__` → `null` (no filter, everyone). Every existing `sales_log`/`bonus_activities` filter in `computeActuals()` was swapped from a single-agent equality check to this resolved-scope membership check — meaning product/premium/activity goals became team/agency-capable as a side effect, not something separately built. Verified split-sale weighting survives this generalization: a split sale between two same-team agents contributes exactly `+1` to the team total (`0.5` per side), not `+2`.

**Call-metric actuals merge `historical_wins` (archived) with live `call_log`**, same strategy as the "Yearly Call Performance archive merge" fix (`api/perf.js`, 2026-09-01) — `call_log` rows are hard-deleted on every Archive & Reset, so an annual goal's period usually spans several already-archived months plus the live one; querying `call_log` alone would silently zero out every archived month. New `computeCallMetricActuals()` (`api/agent-goals.js`) buckets both sources by `(month, agent_id)`, only queries live `call_log` for months `historical_wins` doesn't already cover, and only runs at all when at least one goal in the batch actually tracks a call metric. `call_log.disposition` already stores the classified category (`answered`/`missed`/`voicemail`/`placed`/`internal`/`other`) at insert time (`classifyCalls()`, `api/upload.js`) — not a raw phone-system string — so no re-classification logic was needed, just a direct `disposition IN (...)` filter.

**Direction-aware progress math, no new schema** — `handle_rate` is higher-better (existing `actual/target` ratio shape works as-is); `voicemail_count`/`missed_calls` are lower-better, so `computeIndividualProgressPct()` (`api/_lib/raise-calc.js`) inverts the ratio to `target/actual` for those two, capped at 200% (`LOWER_BETTER_RATIO_CAP`) so a near-zero actual (e.g. 0 missed calls against a target of 5) can't produce an outlier that dominates the plain average this function takes across every active ratio on a goal. Direction is a hardcoded fact about these 3 fixed metrics, not a per-goal choice — unlike `bonus_activity_types.analysis_direction`, which genuinely is per-account-configurable for arbitrary custom activities; this reuses that vocabulary, not its storage.

**Server-side sentinel validation** (`validateAgentId()`, `api/agent-goals.js`, POST only — `agent_id` isn't editable via PATCH): any `__`-prefixed value must exactly match one of the 3 known sentinels, rejected otherwise — cheap insurance against the exact class of bug already fixed once in this codebase (a typo'd `roster_agent_id` silently orphaning a member from Quick-Count visibility, fixed 2026-07-31). A real (non-sentinel) `agent_id` is checked against the roster only when one exists for the account; roster-less accounts (no sales add-on) can't be validated against it and are allowed through.

**Non-writer `GET` visibility extended to sentinel goals** — the existing `.eq('agent_id', memberAgentId || '__unlinked_member__')` scoping would never have returned a `__team_sales__`/`__agency__` row to a bosun/custom member at all, regardless of `is_public`. Extended to `.in('agent_id', [memberAgentId, '__agency__', '__team_'+theirTeam+'__'])`. This only makes the row *fetchable* — it does not by itself gate whether real numbers render (see the client-side gate fix below, 2026-09-04).

**Authoring UI lives with the roster, not the Goals tab** — `_renderScopeGoalsPanel()` (`js/goals.js`) renders "Whole Agency"/"Sales Team"/"Service Team" cards by reusing `renderAgentRosterGoalsSection()` with a synthetic `{agent_id: sentinel}` object (that function only ever reads `.agent_id`, works unmodified). Actually wired into the mounted `js/roster-island.js` React component as of the 2026-09-04 fix below — see "Hit again 2026-09-03" under "Agent Roster is a React island" earlier in this file; the original 2026-09-03 ship of this feature wired it into `js/sales.js`'s dead `renderAgentRoster()` instead and it silently never rendered. The read-only Goals tab (`renderGoalsTab()`, Perf → Goals) needed a separate fix: sentinel-keyed entries never appear in an org chart group's `agentIds` (those only ever list real roster agents), so they're pulled out and rendered before the org-chart branching rather than silently vanishing under any account using one.

**Client-side privacy gate was missing for sentinel goals — `is_public` had no effect (fixed 2026-09-04).** The non-writer `GET` extension above made a private `__team_sales__`/`__agency__` goal *fetchable* by every in-scope member, same as a public one — but nothing downstream ever checked `is_public` before rendering real numbers (unlike individual agents' goals, which never need this check because the server simply never sends another agent's row to a non-writer at all). Reported as "when clicking an agency goal as public, where will it be viewable" — investigating it turned up that Public/Private was cosmetic-only for scope goals: any member in scope saw full actual/target numbers on the Goals tab regardless of the flag, only the badge text changed. Fixed in `renderGoalsTab()`'s `renderAgentBlock()`: for sentinel-scoped goals only (individual-agent goals are untouched, already correctly server-filtered), a non-writer viewer (`!canPrivate`) now sees a locked "🔒 Private" placeholder card (label/period only, no numbers) for any goal with `is_public: false`, instead of `renderGoalCards()`'s normal full-number card. Writers (owner/captain/chief_officer/admin) are unaffected — they always see everything, same as before. Deliberately does **not** touch `renderMyRaiseGoalModule()` (the member's own "🎯 Raise Eligibility" self-view card) — that module intentionally always shows a member their own stake in their team's/agency's raise progress regardless of privacy, the same way an individual's own raise goal always shows to them; the gate above only applies to the generic Goals-tab card list other members' eyes would see. Verified live by creating a private `__team_sales__` goal, confirming the owner view still shows full numbers, then simulating a service-team bosun view (temporarily swapping `_isMember`/`_memberRole`/`_memberAgentId` in a live session and re-calling `renderGoalsTab()`) and confirming it renders the locked placeholder with no real numbers. Test goal deleted after verification.

**Verification performed**: seeded a real 2-sales/1-service test roster, `call_log`/`historical_wins` test data, and confirmed via the live API — team aggregation sums exactly the 2 sales agents' policies (service agent correctly excluded), a split sale between two same-team agents contributes exactly `+1` not `+2`, agency scope sums voicemail count across every agent account-wide, the archived+live month merge produces exactly the hand-computed `handle_rate`/`voicemail_count`/`missed_calls`, the lower-better ratio inversion is correct and zero-safe at the boundary (unit-tested directly: `0/5 → 200%`, `5/5 → 100%`, `10/5 → 50%`), a mistyped sentinel is rejected with 400, and a real pre-existing individual goal's actuals are unchanged. **Live UI confirmed 2026-09-03** (Chrome, `vercel dev`): the "Team & Agency Goals" authoring panel renders in Account → Sales → Team with working +Add/form/save for a real Sales Team goal (including the full raise-config UI appearing correctly for a team-scoped goal), and the Goals tab correctly displays it under a "Sales Team" heading with a real progress bar and a working Raise Eligibility line. **Still not visually verified**: a real non-writer member login (self/team/agency visibility carve-outs remain correct by code + server-scoping review only, not an actual bosun session). Test goal/data cleaned up after verification.

**Operational note — browser HTTP cache during `vercel dev` testing (2026-09-03)**: a page already open in the browser can keep serving an *old* cached copy of a `js/*.js` file even after the underlying file changes on disk and `vercel dev` itself serves the new version — confirmed via a direct `fetch('/js/sales.js', {cache:'no-store'})` returning the updated source while the already-loaded `renderAgentRoster` function in memory still lacked it. `location.reload(true)` (the deprecated force-reload argument) did **not** fix this — modern Chrome ignores it. Reliably fixing it required either a fresh tab/hard-refresh via devtools-level cache clearing, or (used here) re-fetching the script with `cache:'no-store'` and `eval()`-ing it directly into the page to redefine the stale functions in place. Distinct from the already-documented `.vercel/cache` server-side staleness gotcha — this one is purely the browser's own HTTP cache and isn't fixed by `rm -rf .vercel/cache` or restarting `vercel dev`.

**Annual handle_rate silently dropped September's real missed/voicemail, reading ~100% (fixed 2026-09-04).** Reported precisely: "September shows 86%, annual shows 100%." `computeCallMetricActuals()` routes each month in a goal's period to EITHER `historical_months` (archived) OR live `call_log` (not-yet-archived), by month key, never both — but it never excluded the account's own *current* race month (`race_config.current_month`) from being treated as "archived" if a `historical_months` row happened to exist for it (e.g. from an early/partial archive attempt, or any other path that could write that table for a month that's still actively live). When that happens, the goal computation trusts the stale row (answered captured, but `missed`/`voicemail` recorded as 0 at whatever earlier point it was written) instead of re-querying live `call_log` for the real, currently-accumulating numbers — exactly the same *"never trust historical data for the still-live current month"* class of bug already found and fixed once in this codebase for AI Analysis (`api/ai-analysis.js`, see `buildFreshChartData`) — this function just never got the same guard when it was built a day earlier. Fixed by fetching `race_config.current_month` and explicitly skipping any `historical_months` row that matches it, forcing that month to always be freshly queried from `call_log` regardless of what (if anything) `historical_months` has stored for it.

**`api/perf.js`'s own Yearly merge (Call Performance tab) is NOT vulnerable to this same failure mode**, despite doing an analogous archived+live merge — checked directly during this investigation. Its live `call_log` loop populates every period bucket (including `yearly`) *unconditionally* first, and the `historical_wins` merge only ever *adds* archived months on top (`yearly[year].__race.voicemail += h.voicemail`) — there's no either/or month-key routing to silently replace a live month's real numbers with a stale archived one. At worst, a hypothetical stale current-month `historical_wins` row would double-count, not zero out — and in practice `historical_wins` is never written for a still-live month under either normal archive flow (`confirmArchive` blanks `current_month` immediately after writing it) or the out-of-order-upload flow (`archiveCallStatsToHistorical` only ever archives months *before* the current one). No fix was needed here.

**Call Performance table gained a Handle Rate column, team-total only (added 2026-09-04).** Recommended and then built the same session: per-agent rows hardcode VM/Missed to `0` for every period including Yearly (see `api/perf.js`'s `mapToRows()`) — a per-agent Handle Rate would read as a permanent, misleading 100% for literally every agent, every period, by construction. Fixed by computing `handleRate = round(totAnswered / (totAnswered+race.voicemail+race.missed) * 100)` only for the `— TEAM TOTAL —` row (11th array element on every row, `null` for individual agents); `js/perf.js`'s `renderPerf()` renders `null` as `—`. Discovered mid-implementation that a materially-identical figure **already existed** on this same tab — `#perf-handle-ratio` (`index.html`), a small muted text line above the table, computed by `renderPerf()` client-side from whichever row happens to be `— TEAM TOTAL —` in the currently-selected period/date. Verified it live first (monthly and yearly both correctly read 78%, matching everything already cross-validated for this account) before building the column — confirms `api/perf.js`'s Yearly merge genuinely was never broken; only the newer `computeCallMetricActuals()` (goals feature) had the staleness bug above. Both the pre-existing summary line and the new table column now coexist and should always agree, since they read the same `handle_rate` value once `mapToRows` reduces the double-computation from "recompute client-side" to "just read what the server already sent." Verified live: table column shows `—` for every agent row and `78%` on Team Total, for both Monthly and Yearly views; sorting by the new column (`setPerfSort(10)`) works with no errors.

**Call-metric goals gained a coverage-gap indicator — distinguishing "genuinely zero" from "no data source ever existed for this month" (added 2026-09-04).** Follow-up to the handle_rate staleness fix above, from a direct question: "is the handle rate showing inaccurate data because historical data for the year is not saved from prior to Sept?" — then confirmed on the account owner's own real production account: "When I select annual it's only showing this month's missed call logs. Suspect it's because previous months weren't archived until a recent session." Confirmed this is a genuine, **permanent** data-completeness gap, not a bug: `confirmArchive()` hard-deletes `call_log` in full on every archive (see "Archive & Reset Flow" above), so any month that was never run through Archive & Reset — most commonly, every month before an account started actually archiving on a regular cadence — has no recoverable per-call detail anywhere, ever. Before this fix, `computeCallMetricActuals()` silently treated a missing month exactly like a real zero, so a `handle_rate`/`voicemail_count`/`missed_calls` goal built from mostly-absent months looked byte-identical to one built from complete data.

Fixed by having `computeCallMetricActuals()` (`api/agent-goals.js`) track, per goal, which months in its period actually had a data source (`historical_months` or, for the current live month, `call_log`) versus which had neither — attached to the goal's API response as `call_metric_coverage: {totalMonths, coveredMonths, missingMonths}`, present only when incomplete (`coveredMonths < totalMonths`) so a fully-covered goal's response shape is unchanged. `js/goals.js`'s `renderGoalCards()` renders this as an amber warning block under the goal's metric rows — "⚠ Call data incomplete for this period — N of M months available. Missing: <months> (never archived, no data recoverable)." — reusing the exact `"YYYY-MM"` month-key format `computeCallMetricActuals` already produces internally, formatted to `"Sep 2026"` for display. Verified live end-to-end: created a real annual Whole Agency `handle_rate` goal for 2026 (an account whose only real call data is the current live month, April) and confirmed via the actual API response `call_metric_coverage: {totalMonths:12, coveredMonths:1, missingMonths:[11 month keys]}`, then confirmed via the live rendered Goals tab that the warning renders inside the correct goal card, below the `78% / 80% (98%)` Handle Rate progress row, with the correct count and correctly-formatted missing-month list. Test goal deleted after verification.
...
### Location goals + org chart migrations (run — confirmed live 2026-09-02)
```sql
