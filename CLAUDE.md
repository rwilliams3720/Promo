...
**Every chart label rendered as an empty box — no title, axis numbers, category labels, or values, just the bars/gridlines/dots (fixed 2026-08-06).** Reported as "the daily report charts don't have labels on the axis — only boxes." Root cause: `renderChartSvg` set `font-family="Helvetica,Arial,sans-serif"`, but Vercel's serverless Node runtime has **no system fonts installed at all** — none of those names resolve to anything, and `sharp`'s SVG renderer (librsvg) silently draws every `<text>` character as an empty missing-glyph placeholder instead of erroring. This is exactly the failure mode `api/og.js` already solved for its own SVG→PNG render (the "already proven working" pipeline this module's own top comment cites) by embedding a font directly in `og.svg` as a base64 `@font-face` data URI rather than depending on the host having any font installed — `chart-render.js` never got the same treatment and silently shipped with invisible text from day one (2026-07-24). Confirmed live by fetching a real signed chart URL directly and inspecting the PNG: bars/gridlines rendered correctly, every text element was a blank tofu box.

Fix: embed the repo's existing `BebasNeue.ttf` (already used for the site logo/OG image, reused here rather than adding a new font asset per the account owner's preference) the same way `og.svg` does — read + base64-encode once at module load (`fontFaceCss()`, cached across warm invocations, not re-read per request), inject `<defs><style>@font-face{...}</style></defs>` into the generated SVG, and set `font-family` to `'BebasNeue', 'Helvetica', 'Arial', sans-serif` (the fallback chain is now cosmetic-only insurance, not the operative rule — BebasNeue is what actually renders). Verified locally via `vercel dev` against three real chart URLs (bar/line/scatter, including a `$`-formatted premium dataset and a 14-day date-labeled trend) — all render fully legible title/axis/value text now.

**First deploy of the fix above didn't actually fix anything in production — labels were still empty boxes after a hard refresh and a real sent report.** This project uses Vercel's legacy `builds`/`routes` array config (`vercel.json`), where every file that ends up in the deployment is an explicit whitelist entry — nothing is included by default just because it exists in the repo. `og.svg` is listed there as its own `@vercel/static` build target, which is the ONLY reason `api/og.js`'s `readFileSync(path.join(process.cwd(), 'og.svg'))` has ever worked in production. `BebasNeue.ttf` was never added to that list, so `vercel dev` (which serves straight from the working directory, no build/bundling step, hence why local testing passed) could read it fine while the actual deployed function could not — and `fontFaceCss()`'s catch block swallowed that read failure and fell back to `''`, silently reproducing the exact original bug (empty system-font list → blank tofu-box labels) instead of erroring anywhere visible. Fixed by adding `{ "src": "BebasNeue.ttf", "use": "@vercel/static" }` to `vercel.json` alongside `og.svg`, and by making the catch block `console.error` instead of failing silently, so a future regression of this exact kind shows up in Vercel function logs instead of just "the chart looks the same as before."

**Takeaway**: for this project's Vercel config style specifically, "it worked in `vercel dev`" does not prove a new `readFileSync`'d static asset will actually exist in production — always cross-check `vercel.json`'s `builds` array (or now `vercel.ts`, if migrated) has an explicit entry for any new file a serverless function reads off disk, the same way `og.svg` already does, before considering a fix confirmed.
