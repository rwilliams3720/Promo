...
**Where "Blended" raise-goal comparisons actually pull from — a common point of confusion, since there are two separate "agency goal" concepts in this app.** A raise-eligible goal's Blended/Separate `agency_location_id` (see "Annual Raise-Eligibility Tracker" above) reads targets from **this table** — `sales_locations.goal_count`/`goal_premium`/`goal_count_annual`/`goal_premium_annual`, i.e. the per-location Office Goals set in Account → Sales → Locations → Edit. It does **not** read from the separate "Whole Agency" `agent_goals` sentinel row (Account → Sales → Team → Team & Agency Goals) at all — that's an entirely independent goal-tracking mechanism with its own metric set (which can itself independently be flagged raise-eligible, but is never consulted as the "agency side" of a *different* goal's Blended/Separate comparison). The "Agency Location" and "Combination Mode" tooltips in the raise-goal authoring form (`js/goals.js`, `buildRaiseGoalHtml`) now state this explicitly, added after a report asking "confirm where the blended goal is grabbing from" surfaced that this wasn't obvious from the UI alone.
