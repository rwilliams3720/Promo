...
**Non-writer `GET` visibility extended to sentinel goals** — the existing `.eq('agent_id', memberAgentId || '__unlinked_member__')` scoping would never have returned a `__team_sales__`/`__agency__` row to a bosun/custom member at all, regardless of `is_public`. Extended to `.in('agent_id', [memberAgentId, '__agency__', '__team_'+theirTeam+'__'])`. No additional client-side visibility gate was needed beyond this — unlike the Race-tab pill fix above, nothing downstream (`renderGoalCards`/`renderMyRaiseGoalModule`) does its own extra `is_public` filtering on top of an already-role-scoped server fetch, so once the row is fetchable its real numbers render, same as an individual's own goal always has.

**Authoring UI lives with the roster, not the Goals tab** — `_renderScopeGoalsPanel()` (`js/goals.js`) renders "Whole Agency"/"Sales Team"/"Service Team" cards by reusing `renderAgentRosterGoalsSection()` with a synthetic `{agent_id: sentinel}` object (that function only ever reads `.agent_id`, works unmodified). Wired into `renderAgentRoster()` (`js/sales.js`, `#agent-roster-list` — the live Account → Sales → Team surface; `js/roster-island.js` is a parallel React/htm implementation of the same roster UI that's never actually mounted anywhere, confirmed via a repo-wide search for its own mount function — dead code, not touched). The read-only Goals tab (`renderGoalsTab()`, Perf → Goals) needed a separate fix: sentinel-keyed entries never appear in an org chart group's `agentIds` (those only ever list real roster agents), so they're pulled out and rendered before the org-chart branching rather than silently vanishing under any account using one.
