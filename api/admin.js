...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      const { data, error } = await supabase
        .from('accounts')
        .select('user_id,email,company_name,contact_name,phone,plan,agent_count,referral_source,status,is_admin,notes,trial_ends_at,paid_through,stripe_customer_id,created_at,last_login,has_sales_addon,has_member_analysis,member_analysis_count')
  ...
  if (req.method === 'PATCH') {
    ...
    const allowed = ['status','notes','paid_through','is_admin','plan','agent_count','trial_ends_at','stripe_customer_id','timezone','report_hour','has_sales_addon','has_member_analysis','member_analysis_count'];
    const update  = {};
